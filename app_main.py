...
class AppMain:
    ...
    def run(self) -> None:
        ...
        lock_path = f"app.{config_file_name}.lock"
        lock = FileLock(lock_path)

        # Define read and write functions to be passed as object data
        def read_file():
        ...
        service_monitor = ServiceMonitor(args.friendly_name, self.configuration, lock, write_file, read_file)
        ...
        hive_service = HiveReceiverService(local_node.friendly_name, local_node.ip_address, local_node.port_number, self.hive_node_manager, self.inbound_message_queue, self.outbound_message_queue, service_monitor, lock, write_file)
        ...
        self.cli_command_processor = CliCommandProcessor(self.hive_node_manager, self.outbound_message_queue, self.inbound_message_queue, args.friendly_name, lock)
