...
class CliCommandProcessor:
    ...
    def __init__(self, hive_node_manager: HiveNodeManager, outbound_message_queue: MessageQueue, inbound_message_queue: MessageQueue, local_node, lock):
        """
    ...
    def display_config(self) -> None:
        """
        Display the config profile for the entire hive
        """
        ...
        if os.path.exists(config_file_name):
            try:
                ...
                pass
        print("")
        for node in check_config:
            print(f"{node}:")
            local_config = check_config[self.local_name][1]
            for key in local_config:
                print(f"{key} - {local_config[key][1]} - {local_config[key][3]}")
            print("\n")

    def display_local_config(self) -> None:
        """
        Display the config for the local node with service status
        """
        config_file_name = f"{self.local_name}.json"
        if os.path.exists(config_file_name):
            try:
        ...
        print("")
        for key in local_config:
            print(f"{local_config[key][0]} - {local_config[key][6]} - {key} - {local_config[key][1]} - {local_config[key][3]}")
        print("")
